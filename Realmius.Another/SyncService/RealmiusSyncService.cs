    ...
    public class RealmiusSyncService : IRealmiusSyncService
    {
        ...
        private static RealmConfiguration RealmiusConfiguration => new RealmConfiguration(RealmiusDbPath)
        {
            ...
            ObjectClasses = new[]
            {
                ...
                typeof(RealmModels.UploadFileInfo),
        ...
        private void Initialize()
        {
            ...
            using (var realmius = CreateRealmius())
            {
                ...
                var subscribe2 = _strongReferencedRealmius.All<RealmModels.UploadFileInfo>().SubscribeForNotifications(UploadFileChanged);
        ...
        private void UploadFileChanged(IRealmCollection<RealmModels.UploadFileInfo> sender, ChangeSet changes, Exception error)
        {
            UploadFiles();
        }
        ...
        public virtual async Task UploadFiles()
        {
            ...
            using (var realmius = CreateRealmius())
            {
                ...
                try
                {
                    var files = realmius.All<RealmModels.UploadFileInfo>().Where(x => !x.UploadFinished).OrderBy(x => x.Added);
                    ...
                    var fullPath = Path.Combine(CrossStorage.FileSystem.LocalStorage.FullPath, path);
                    var fileContent = CrossStorage.FileSystem.GetFileFromPath(fullPath);
                    var stream = fileContent.Open(FileAccess.Read);
                    ...
                    using (var content = new MultipartFormDataContent())
                    {
                        ...
                        {
                            var file2 = realmius2.Find<RealmModels.UploadFileInfo>(id);
                            realmius2.Write(
                            () =>
...
#pragma warning restore 4014
            ...
            string additionalInfo = null)
        {
            ...
            Uri uri2 = new Uri(CrossStorage.FileSystem.LocalStorage.FullPath + "/");
            string relativePath = uri2.MakeRelativeUri(uri1).ToString();

            var fileInfo = new RealmModels.UploadFileInfo
            {
